import React from 'react';

const Services = () => {
  return (
    <div id="services" className="text-white font-siliguri">
      <div>
        <h1 className="text-center text-2xl md:text-4xl font-semibold">এক নজরে আমাদের <span className="text-cyan-500">সেবাসমূহ</span></h1>
      </div>
      <>
        <div className="px-4 py-0 md:py-16 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-0 lg:py-20">
          <div className="grid row-gap-8 sm:row-gap-0 sm:grid-cols-2 lg:grid-cols-3">
            <div className="p-8 border-b sm:border-r">
              <div className="max-w-md text-center">
                <div className="flex items-center justify-center w-16 h-16 mx-auto mb-4 rounded-full sm:w-16 sm:h-16">
                  <svg
                    className="w-8 h-8 text-deep-purple-accent-400 sm:w-12 sm:h-12"
                    stroke="currentColor"
                    viewBox="0 0 52 52"
                  >
                    <polygon
                      strokeWidth="3"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      fill="none"
                      points="29 13 14 29 25 29 23 39 38 23 27 23"
                    />
                  </svg>
                </div>
                <h6 className="mb-2 font-semibold leading-5">ফরম পূরণ</h6>
                <p className="mb-3 text-sm">
                  স্কুলের ভর্তি ফরম, কলেজের ভর্তি ফরম, বিশ্ববিদ্যালয়ের ভর্তি ফরম ।
                </p>
              </div>
            </div>
            <div className="p-8 border-b lg:border-r">
              <div className="max-w-md text-center">
                <div className="flex items-center justify-center w-16 h-16 mx-auto mb-4 rounded-full sm:w-16 sm:h-16">
                  <svg
                    className="w-8 h-8 text-deep-purple-accent-400 sm:w-12 sm:h-12"
                    stroke="currentColor"
                    viewBox="0 0 52 52"
                  >
                    <polygon
                      strokeWidth="3"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      fill="none"
                      points="29 13 14 29 25 29 23 39 38 23 27 23"
                    />
                  </svg>
                </div>
                <h6 className="mb-2 font-semibold leading-5">কম্পিউটার মেরামত</h6>
                <p className="mb-3 text-sm">
                  ডেস্কটপ মেরামত, ল্যাপটপ মেরামত ।
                </p>
              </div>
            </div>
            <div className="p-8 border-b sm:border-r lg:border-r-0">
              <div className="max-w-md text-center">
                <div className="flex items-center justify-center w-16 h-16 mx-auto mb-4 rounded-full sm:w-16 sm:h-16">
                  <svg
                    className="w-8 h-8 text-deep-purple-accent-400 sm:w-12 sm:h-12"
                    stroke="currentColor"
                    viewBox="0 0 52 52"
                  >
                    <polygon
                      strokeWidth="3"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      fill="none"
                      points="29 13 14 29 25 29 23 39 38 23 27 23"
                    />
                  </svg>
                </div>
                <h6 className="mb-2 font-semibold leading-5">চাকরির আবেদন</h6>
                <p className="mb-3 text-sm">
                  চাকরির অনলাইন আবেদন, অফলাইন আবেদন ।
                </p>
              </div>
            </div>
            <div className="p-8 border-b lg:border-b-0 lg:border-r">
              <div className="max-w-md text-center">
                <div className="flex items-center justify-center w-16 h-16 mx-auto mb-4 rounded-full sm:w-16 sm:h-16">
                  <svg
                    className="w-8 h-8 text-deep-purple-accent-400 sm:w-12 sm:h-12"
                    stroke="currentColor"
                    viewBox="0 0 52 52"
                  >
                    <polygon
                      strokeWidth="3"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      fill="none"
                      points="29 13 14 29 25 29 23 39 38 23 27 23"
                    />
                  </svg>
                </div>
                <h6 className="mb-2 font-semibold leading-5">তথ্য সংশোধন ও আবেদন</h6>
                <p className="mb-3 text-sm">
                  নতুন জন্ম নিবন্ধন আবেদন ও সংশোধন, নতুন জাতীয় পরিচয়পত্রের আবেদন ও সংশোধন, টিন (TIN) সার্টিফিকেট আবেদন ।
                </p>
              </div>
            </div>
            <div className="p-8 border-b sm:border-b-0 sm:border-r">
              <div className="max-w-md text-center">
                <div className="flex items-center justify-center w-16 h-16 mx-auto mb-4 rounded-full sm:w-16 sm:h-16">
                  <svg
                    className="w-8 h-8 text-deep-purple-accent-400 sm:w-12 sm:h-12"
                    stroke="currentColor"
                    viewBox="0 0 52 52"
                  >
                    <polygon
                      strokeWidth="3"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      fill="none"
                      points="29 13 14 29 25 29 23 39 38 23 27 23"
                    />
                  </svg>
                </div>
                <h6 className="mb-2 font-semibold leading-5">গ্রাফিক্স ও প্রিন্টিং</h6>
                <p className="mb-3 text-sm">
                  অত্যাধুনিক ব্যানার ডিজাইন, পোস্টার ডিজাইন, লিফলেট, ভিজিটিং কার্ড, ক্যাশ মেমো, টি-শার্ট ডিজাইন, ছবি প্রিন্ট, মগ প্রিন্ট ।
                </p>
              </div>
            </div>
            <div className="p-8">
              <div className="max-w-md text-center">
                <div className="flex items-center justify-center w-16 h-16 mx-auto mb-4 rounded-full sm:w-16 sm:h-16">
                  <svg
                    className="w-8 h-8 text-deep-purple-accent-400 sm:w-12 sm:h-12"
                    stroke="currentColor"
                    viewBox="0 0 52 52"
                  >
                    <polygon
                      strokeWidth="3"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      fill="none"
                      points="29 13 14 29 25 29 23 39 38 23 27 23"
                    />
                  </svg>
                </div>
                <h6 className="mb-2 font-semibold leading-5">অন্যান্য সেবা</h6>
                <p className="mb-3 text-sm">
                  দলিল লেখা, বিমানের টিকিট বুকিং, ই-মেইল, পাসপোর্টের আবেদন ইত্যাদি যাবতীয় সেবা ।
                </p>
              </div>
            </div>
          </div>
        </div>
      </>
    </div>
  );
};

export default Services;